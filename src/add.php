...
if($_POST){
    ...
    if( isset( $_POST['nombre'] ) && ($_FILES && isset($_FILES['imagen']) && 
    $_FILES['imagen']['error'] === UPLOAD_ERR_OK && $_FILES['imagen']['size'] > 0) )
    {
        $nombreValido = htmlspecialchars( trim( $_POST['nombre'] ) );
        $tipoMIME = $_FILES['imagen']['type'];
        $imagen = $_FILES['imagen'];
        $ext =  mb_substr($tipoMIME,6);
        $fechaAct = date_create();
        if(str_contains( $tipoMIME,"image" ) ){
            $rutaFichero = "./imagenes/".basename(date_timestamp_get($fechaAct).".".$ext);
            $seHaSubido = move_uploaded_file($_FILES['imagen']['tmp_name'], $rutaFichero);
            $nomUser = $_SESSION['usuario'];
            $usuario = usuarioEnUso($nomUser);
            $numIDUser = $usuario[0]['id'];
            

            insertarImagen($nombreValido,$rutaFichero, $numIDUser);
        }else{
            echo "<h2>ERROR: Tipo de archivo no permitido</h2>";
        }
    }
...
?>
        <ul>
            ...
            <li><strong>Añadir Imágenes</strong></li>
